...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/octos/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/octos/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/octos/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        pushd $T > /dev/null
        vendor/octos/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        T=$(gettop)
        pushd $T > /dev/null
        vendor/octos/build/tools/roomservice.py $product true
        popd > /dev/null
    fi
    if [ $? -ne 0 ]
    then
    ...
    if (echo -n $product | grep -q -e "^octos_") ; then
       CUSTOM_BUILD=$(echo -n $product | sed -e 's/^octos_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
. $ANDROID_BUILD_TOP/vendor/octos/build/envsetup.sh
